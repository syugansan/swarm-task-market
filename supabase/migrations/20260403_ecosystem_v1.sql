-- Ecosystem schema v1
-- Date: 2026-04-03
-- Goal: add the first public ecosystem data layer for hives, tasks, matches, deliveries, and feedback.
-- Strategy: additive migration only. Keep the current skills / agent_profiles / escrow / q-score structures intact.
...
CREATE TABLE IF NOT EXISTS hives (
  ...
  owner_agent_id UUID REFERENCES agent_profiles(id) ON DELETE SET NULL,
  slug TEXT UNIQUE,
  ...
  website_url TEXT,
  contact_url TEXT,
  lane TEXT NOT NULL DEFAULT 'lab' CHECK (lane IN ('lab', 'verified', 'hybrid')),
  verification_status TEXT NOT NULL DEFAULT 'pending' CHECK (verification_status IN ('pending', 'trial', 'verified', 'suspended')),
...
-- 2. Hive <-> Skill links
-- ============================================================
-- Note: skills schema currently uses both id and skill_id conventions in app code.
-- To stay compatible, keep this table loosely coupled through skill_id UUID without a hard FK.

CREATE TABLE IF NOT EXISTS hive_skills (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  hive_id UUID NOT NULL REFERENCES hives(id) ON DELETE CASCADE,
  skill_id UUID NOT NULL,
  relationship_type TEXT NOT NULL DEFAULT 'published' CHECK (relationship_type IN ('published', 'inherited', 'adapted', 'verified')),
  source_standard TEXT DEFAULT 'native' CHECK (source_standard IN ('native', 'anthropic-skills', 'clawhub', 'external', 'native')),
  inheritance_ready BOOLEAN NOT NULL DEFAULT TRUE,
  verification_status TEXT NOT NULL DEFAULT 'pending' CHECK (verification_status IN ('pending', 'tested', 'verified', 'rejected')),
  note TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE (hive_id, skill_id, relationship_type)
);
...
-- 3. Tasks
-- ============================================================
-- If the tasks table already exists, extend it with ecosystem fields.
-- If it does not exist, create a version compatible with the current publish page.
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  lane TEXT NOT NULL DEFAULT 'lab' CHECK (lane IN ('lab', 'verified', 'hybrid')),
  intake_source TEXT NOT NULL DEFAULT 'web' CHECK (intake_source IN ('web', 'api', 'internal', 'partner')),
  summary TEXT,
  brief_md TEXT,
  budget_currency TEXT DEFAULT 'USD',
  visibility TEXT NOT NULL DEFAULT 'public' CHECK (visibility IN ('public', 'unlisted', 'private')),
  selected_hive_id UUID REFERENCES hives(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS task_matches (
  ...
  hive_id UUID NOT NULL REFERENCES hives(id) ON DELETE CASCADE,
  match_status TEXT NOT NULL DEFAULT 'proposed' CHECK (match_status IN ('proposed', 'shortlisted', 'accepted', 'rejected', 'completed')),
  fit_score NUMERIC(5,2),
  match_reason TEXT,
  matched_by TEXT DEFAULT 'system' CHECK (matched_by IN ('system', 'manual', 'hybrid')),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE (task_id, hive_id)
);
...
CREATE TRIGGER trigger_task_matches_updated_at
BEFORE UPDATE ON task_matches
FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TABLE IF NOT EXISTS deliveries (
  ...
  submitted_by_agent_id UUID REFERENCES agent_profiles(id) ON DELETE SET NULL,
  title TEXT NOT NULL,
  summary TEXT,
  artifact_url TEXT,
  artifact_type TEXT DEFAULT 'link' CHECK (artifact_type IN ('link', 'text', 'file', 'repo', 'mixed')),
  delivery_status TEXT NOT NULL DEFAULT 'submitted' CHECK (delivery_status IN ('submitted', 'under_review', 'accepted', 'rejected', 'revised')),
...
CREATE TABLE IF NOT EXISTS feedback (
  ...
  outcome TEXT NOT NULL DEFAULT 'positive' CHECK (outcome IN ('positive', 'neutral', 'negative')),
  feedback_text TEXT,
  feedback_type TEXT NOT NULL DEFAULT 'delivery' CHECK (feedback_type IN ('delivery', 'collaboration', 'verification')),
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ============================================================
-- 7. Public views for the website
-- ============================================================

CREATE OR REPLACE VIEW hive_public_profiles AS
SELECT
  h.id,
  h.slug,
  h.display_name,
  ...
  h.status,
  h.connected_at,
  COUNT(DISTINCT hs.id) AS listed_skills,
  COALESCE(ROUND(AVG(f.rating)::numeric, 2), 0) AS avg_rating,
  COUNT(DISTINCT tm.id) FILTER (WHERE tm.match_status = 'completed') AS completed_matches
FROM hives h
LEFT JOIN hive_skills hs ON hs.hive_id = h.id
LEFT JOIN task_matches tm ON tm.hive_id = h.id
LEFT JOIN feedback f ON f.hive_id = h.id
GROUP BY h.id;

CREATE OR REPLACE VIEW task_lane_overview AS
SELECT
  t.id,
  t.title,
  t.task_type,
  ...
  MAX(d.submitted_at) AS last_delivery_at
FROM tasks t
LEFT JOIN task_matches tm ON tm.task_id = t.id
LEFT JOIN deliveries d ON d.task_id = t.id
GROUP BY t.id;
