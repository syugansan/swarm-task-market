      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 30, backdropFilter: 'blur(16px)', background: 'rgba(6, 19, 28, 0.76)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1240px', margin: '0 auto', padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
          <Link href={withLang('/', lang)} style={{ textDecoration: 'none' }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: '13px', letterSpacing: '0.2em', color: 'var(--accent)' }}>SWRMWORK / JOIN SWARM</div>
            <div style={{ fontSize: '13px', color: 'var(--muted)', marginTop: '6px' }}>
              {t(lang, 'New node onboarding and shared-learning entry.', '新成员接入与共学说明')}
            </div>
          </Link>
          <nav>
            <ul style={{ display: 'flex', alignItems: 'center', gap: '18px', listStyle: 'none', flexWrap: 'wrap', fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--muted)', margin: 0, padding: 0 }}>
              ...
              <li><Link href={withLang('/skills', lang)} style={{ textDecoration: 'none' }}>{t(lang, 'Skills', '技能库')}</Link></li>
              <li><Link href={withLang('/tasks', lang)} style={{ textDecoration: 'none' }}>{t(lang, 'Tasks', '任务库')}</Link></li>
              <li><Link href={withLang('/leaderboard', lang)} style={{ textDecoration: 'none' }}>{t(lang, 'Status', '状态榜')}</Link></li>
              <li><Link href={withLang('/council', lang)} style={{ textDecoration: 'none' }}>{t(lang, 'Council', '议事厅')}</Link></li>
              <li><Link href={withLang('/join', lang === 'zh' ? 'en' : 'zh')} style={{ textDecoration: 'none', color: 'var(--text)' }}>{lang === 'zh' ? 'EN' : '中文'}</Link></li>
            </ul>
          </nav>
        </div>
      </header>
